...
(1, 'Harini', '21pt05@psgtech.ac.in', 'About Chapter', 'Insert new chapter in CSS course');
...
(1, 'Harini', 'P', 'Paramanandham', '21pt05@psgtech.ac.in', 'female', 'itsharryni', 'asdf');
...
('Good experience', 'sashyyy', '21pt24@psgtech.ac.in');
...
('HTML Complete Reference', 4, 'Sash h', 'door no 35', 'Coimbatore', 641014, 'Tamilnadu', '21pt24@psgtech.ac.in', '9566749596');
...
(1, 13, 'sashyyy', 'What is HTML?'),
(3, 13, 'sashyyy', 'What is CSS?');
...
(13, 'Sashi', 'H', 'female', '21pt24@psgtech.ac.in', 'sashyyy', '1234'),
(14, 'Shwetha', 'S', 'female', 'shwe2003@gmail.com', 'shadow', '1234');
...
--Trigger to update the user's name in que table when their name is updated in user_info table
DELIMITER $$
CREATE TRIGGER `user_info_update` AFTER UPDATE ON `user_info` FOR EACH ROW
BEGIN
    UPDATE `que` SET `UserName` = CONCAT(NEW.FirstName, ' ', NEW.LastName) WHERE `UserID` = NEW.User_id;
END$$
DELIMITER ;

--if a new user registers with an already existing username it should give an error message
DELIMITER $$
CREATE TRIGGER `user_info_insert` BEFORE INSERT ON `user_info` FOR EACH ROW
BEGIN
    DECLARE username_count INT;
    SELECT COUNT(*) INTO username_count FROM `user_info` WHERE `UserName` = NEW.UserName;
...
--trigger for checking the password in user_info is atleast 6 characters long
DELIMITER $$
